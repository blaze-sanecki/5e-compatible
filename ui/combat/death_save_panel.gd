...
func _build_ui() -> void:
	...
	s_label.add_theme_color_override("font_color", Color(0.3, 0.9, 0.3))
	success_row.add_child(s_label)
	for i in 3:
		var circle := Label.new()
		circle.text = "O"
		circle.add_theme_font_size_override("font_size", 16)
		circle.add_theme_color_override("font_color", Color(0.3, 0.3, 0.3))
	...
	f_label.add_theme_color_override("font_color", Color(0.9, 0.3, 0.3))
...
func _on_roll_pressed() -> void:
	...
	if result.get("revived", false):
		...
		_result_label.add_theme_color_override("font_color", Color(0.8, 0.2, 0.2))
		_roll_button.disabled = true
		await get_tree().create_timer(1.5).timeout
		hide_panel()
		_combat_manager.end_current_turn()
...
func _update_circles(combatant: CombatantData) -> void:
	for i in 3:
		if i < combatant.death_save_successes:
			_success_labels[i].add_theme_color_override("font_color", Color(0.3, 1.0, 0.3))
		else:
			_success_labels[i].add_theme_color_override("font_color", Color(0.3, 0.3, 0.3))

		if i < combatant.death_save_failures:
			_failure_labels[i].add_theme_color_override("font_color", Color(1.0, 0.3, 0.3))
		else:
			_failure_labels[i].add_theme_color_override("font_color", Color(0.3, 0.3, 0.3))
